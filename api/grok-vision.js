...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey) {
      return res.status(401).json({ error: '未設定 xAI API Key。請到設定頁填入，或在 Vercel 設定 XAI_API_KEY。' });
    }
    ...
    const allowedModels = [
      'grok-2-vision-latest', 'grok-2-vision', 'grok-4.5',
      'grok-4-fast-non-reasoning', 'grok-4-fast-reasoning', 'grok-2-latest'
    ];
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: msg, modelUsed: model });
    }
